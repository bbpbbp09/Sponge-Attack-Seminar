...
def plot_comparison():
    ...
    for model_name, path in MODELS.items():
        ...
        # Calculate some simple stats for the table
        # We assume data is normalized, so we look at variance/max/min
        row = {
            'Model': model_name,
            'Min Val': data.min(),
            'Max Val': data.max(),
    ...
    ax_table.axis('off')
    ax_table.set_title("Adversarial Input Statistics")
    ...
    for i, (model_name, path) in enumerate(MODELS.items()):
        ...
        plt.title(f"{model_name} - Full Input Heatmap (Features x Time)")
    ...
    plt.savefig("attack_comparison_summary.png", dpi=150)
    print("Saved plot: attack_comparison_summary.png")
    
    # Print Table
    df = pd.DataFrame(stats)
    print("\nAdversarial Input Statistics:")
